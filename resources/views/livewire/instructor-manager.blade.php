<div class="w-full">
        @if($showDeleteModal)
                <x-modal>
                    <div class="p-6 bg-blue-200 text-black">
                        <h2 class="text-lg font-semibold mb-4">Are you sure you want to delete this instructor?</h2>
    ...
    <div class="w-3/4 flex justify-between mx-auto mt-10">
        ...
        <div>
            <a href="{{ route ('instructor') }}"><x-button class="bg-sky-300"><i class="fa-solid fa-plus"></i>Add Instructor</x-button></a>
        </div>
    </div>
                ...
                <tr class="bg-sky-100 text-black rounded-lg">
                    ...
                    <th class="py-2">
                        <div class="inline-block text-left">
                            <button id="dropdownButton-{{ $user->id }}" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center" onclick="toggleDropdown({{ $user->id }})">
                            ...
                            <div id="dropdownMenu-{{ $user->id }}" class="z-10 hidden bg-white rounded-lg shadow dark:bg-gray-700 absolute transform -translate-x-1/3 mt-2">
                                <div class="bg-blue-300 p-3">
                                    <x-button wire:click="viewInstructor ({{$user->id}})">View</x-button>
                                    <x-button onclick="window.location='{{ route('instructor.edit', $user->id) }}'">Edit</x-button>
