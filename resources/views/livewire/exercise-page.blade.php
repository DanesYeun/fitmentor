<div class="w-full">
        ...
        @if($showDeleteModal)
                <x-modal>
                    <div class="p-6 bg-gray-200 text-black">
                        <h2 class="text-lg font-semibold mb-4">Are you sure you want to delete this exercise?</h2>
    ...
    <div class="w-3/4 flex justify-between mx-auto mt-10">
        ...
        <div>
            <a href="{{ route ('exercise-maker') }}"><x-button class="bg-gray-400"><i class="fa-solid fa-plus"></i>Add Exercise</x-button></a>
        </div>
    </div>
    ...
    <table class="w-full mx-auto rounded-lg">
            <thead>
                <tr class="bg-gray-200 text-black rounded-lg">
                    <th class="py-2 w-64">Exercise Name</th>
                    <th class="py-2 w-64">Preparation</th>
                    <th class="py-2 w-64">Execution</th>
            ...
            <tbody>
                ...
                <tr class="bg-gray-100 text-black rounded-lg">
                    <th class="py-2">{{ $exercise->name }}</th>
                    <th class="py-2">{{ $exercise->preparation }}</th>
                    <th class="py-2">{{ $exercise->execution }}</th>
                    <th class="py-2">
                        <div class="inline-block text-left">
                            <button id="dropdownButton-{{ $exercise->id }}" class="text-white bg-gray-700 hover:bg-gray-800 focus:ring-4 focus:outline-none focus:ring-gray-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center" onclick="toggleDropdown({{ $exercise->id }})">
                            ...
                            <div id="dropdownMenu-{{ $exercise->id }}" class="z-10 hidden bg-white rounded-lg shadow dark:bg-gray-700 absolute transform -translate-x-1/3 mt-2">
                                <div class="bg-gray-300 p-3">
                                    <x-button class="bg-gray-500 shadow-lg" wire:click="viewProgram ({{$exercise->id}})">View</x-button>
                                    <x-button class="bg-gray-500 shadow-lg" onclick="window.location='{{ route('exercise.edit', $exercise->id) }}'">Edit</x-button>
                                    <x-button class="bg-gray-500 shadow-lg" wire:click="confirmDelete({{ $exercise->id }})">Delete</x-button>
                                </div>
                            </div>
                        </div>
                    </th>
                ...
                <x-modal>
                    <div class="w-full mx-auto p-5 bg-gray-100 rounded-lg shadow-lg text-black">
                        <div class="mx-auto w-1/3">
                            <h1 class="text-xl font-bold mb-5">Exercise Details</h1>
                        </div>
                        <div class="mt-5">
