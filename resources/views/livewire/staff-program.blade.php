<div class="w-full">
    ...
    <div class="w-3/4 flex justify-between mx-auto mt-10">
        ...
        <div>
            <a href="{{ route ('program-maker') }}"><x-button class="bg-sky-400"><i class="fa-solid fa-plus"></i>Add Program</x-button></a>
        </div>
    </div>
                ...
                <tr class="bg-sky-100 text-black rounded-lg">
                    ...
                    <th class="py-2">
                        <div class="inline-block text-left">
                            <button id="dropdownButton-{{ $program->id }}" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center" onclick="toggleDropdown({{ $program->id }})">
                            ...
                            <div id="dropdownMenu-{{ $program->id }}" class="z-10 hidden bg-white rounded-lg shadow dark:bg-gray-700 absolute transform -translate-x-1/3 mt-2">
                                <div class="bg-blue-300 p-3">
                                    <x-button class="bg-sky-500 shadow-lg" wire:click="viewProgram ({{$program->id}})">View</x-button>
                                    <x-button class="bg-sky-500 shadow-lg" onclick="window.location='{{ route('program.edit', $program->id) }}'">Edit</x-button>
                                    <x-button class="bg-sky-500 shadow-lg" wire:click="confirmDelete({{ $program->id }})">Delete</x-button>
                                </div>
                            </div>
                        </div>
                    </th>
