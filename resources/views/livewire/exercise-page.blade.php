<div class="w-full">
    ...
    <table class="w-full mx-auto rounded-lg">
                ...
                <tr class="bg-sky-100 text-black rounded-lg">
                    ...
                    <th class="py-2">
                        <div class="inline-block text-left">
                            <button id="dropdownButton-{{ $exercise->id }}" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center" onclick="toggleDropdown({{ $exercise->id }})">
                            ...
                            <div id="dropdownMenu-{{ $exercise->id }}" class="z-10 hidden bg-white rounded-lg shadow dark:bg-gray-700 absolute transform -translate-x-1/3 mt-2">
                                <div class="bg-blue-300 p-3">
                                    <x-button class="bg-sky-500 shadow-lg" wire:click="viewProgram ({{$exercise->id}})">View</x-button>
                                    <x-button class="bg-sky-500 shadow-lg" onclick="window.location='{{ route('exercise.edit', $exercise->id) }}'">Edit</x-button>
                                    <x-button class="bg-sky-500 shadow-lg" wire:click="confirmDelete({{ $exercise->id }})">Delete</x-button>
                                </div>
                            </div>
                        </div>
                    </th>
        ...
        @if($showModal)
                <x-modal>
                    <div class="w-full mx-auto p-5 bg-sky-400 rounded-lg shadow-lg text-black">
